...
struct fifo_head{
	...
	struct fifo* last;
	atomic_t count;
	spinlock_t lock_last;
};
struct queue_thread_data{
	int cpuid;
	struct fifo_head fifo;
	wait_queue_head_t queue_thread_wait;
//struct tasklet_struct tasklet_queue;
struct workqueu_struct* workqueue;	
struct mutex queue_thread_mutex;
};
...
/*static inline pte_t pte_mkcacheable(pte_t pte){
	return pte_set_flags(pte, _PAGE_PCD|_PAGE_NCACHE);
}
static inline pte_t pte_cleancacheable(pte_t pte){
	return pte_clean_flags(pte, _PAGE_PCD|_PAGE_NCACHE);
}*/
void init_fault_fifo(struct fifo* fifo);
int fault_fifi_out(struct fifo_head* fifo);
extern struct queue_thread_data *queue_paras;
extern struct kmem_cache* sclock_fault_entry_cache;
int try_switch_NCache(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address,unsigned long pfn,pte_t orig_pte,bool invalid);
