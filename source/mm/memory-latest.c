...
reuse:
		...
		if(old_page->coa_head){
			...
			coa_parent=get_original(coa_entry);
			if(!mutex_trylock(&coa_parent->lock)){
				pte_unmap_unlock(page_table, ptl);
			...
			printk("cow for coa at %lx",address);
			if(coa_entry->parent_head){
				...
				//		mutex_lock(&coa_entry->lock);
				del_coa_parent_slow(coa_entry);
				//	mutex_unlock(&coa_entry->lock);
			}
			mutex_unlock(&coa_parent->lock);
...
static int	add_page_into_children_pages(struct sclock_coa_parent* coa_parent,struct page* page,struct pid_namespace *pid_ns){
	...
	if(coa_parent->copy_number>=0)	
	  coa_parent->copy_number=coa_parent->copy_number+1;
	else
	  coa_parent->copy_number=1;
...
unsigned long global_count_coa=0,global_count_coa_early=0;
unsigned long coa_time1,coa_time=0;
struct sclock_coa_parent *get_original(struct sclock_coa_parent *coa_parent){
	if(coa_parent->parent_head==NULL){
		return coa_parent;
	}
	return list_entry_rcu(coa_parent->parent_head,struct sclock_coa_parent,head);
}
...
not_coa:
	...
	if(coa_parent){
		mutex_lock(&(get_original(coa_parent)->lock));
		list_for_each_entry(coa_children,&coa_parent->head,head){
			if(coa_children->owner==pid_ns){
				unsigned long coa_pfn=coa_children->pfn;
				if((coa_pfn!=COA_DEL)&&(coa_pfn!=COA_SKIP)){
					coa_children->pfn=COA_SKIP;
					new_page=pfn_to_page(coa_pfn);
					if(page_mapped(new_page)&&pages_identical(new_page,old_page)){
						not_new=true;
						printk("reuse the container's exclusive copy at %lx ->page%lx\n",address,page_to_pfn(new_page));
						page_cache_get(new_page);
						//	__SetPageUptodate(new_page);
						mutex_unlock(&(get_original(coa_parent)->lock));
						goto new_page_got;
					}
				}
			}
...
new_page_got:
	...
	if (likely((pte_same(*page_table, orig_pte)))) {
		...
		entry.pte&=~(_PAGE_ISOLATION|_PAGE_RW);
		if(not_new){
			lock_page(new_page);
		//	page_cache_get(new_page);
...
#define PAGE_NCACHE_ALL (_PAGE_NCACHE|_PAGE_CACHE_UC)	
		  ...
		  if((mm->def_flags&VM_NCACHE)&&(task=mm->owner))
			if(ns_of_pid(task_pid(task))==pid_ns_ref){
				...
				if(pte)
				  if(pte_present(*pte)){
					  /*	if(pte->pte|_PAGE_ACCESSED){
					  //printk("PAGE_ACCESSED in file");
					  ret= -1;// cannot marked as UC...
					  goto unlock;
					  }*/
					  ret++;
					  pte->pte|=flags;
...
//	page_one=NULL;
	...
	spin_lock(&(pid_ns->sclock_lock[set_number]));
	list_for_each_entry_rcu(sclock_entry,lru_head,sclock_lru){
...
add:
	if(addNewLRU){
		if(findNToDel>0){
			...
			list_add_tail_rcu(&sclock_entry->sclock_lru,lru_head);
			spin_unlock(&(pid_ns->sclock_lock[set_number]));
			//printk("replace\n");
		}else{
			...
			list_add_tail_rcu(&sclock_one->sclock_lru,lru_head);
			spin_unlock(&(pid_ns->sclock_lock[set_number]));
			atomic_inc(counter);
...
static DEFINE_MUTEX(quick_pte_map_mutex);
...
struct sclock_page_pte_map * sclock_pte_map_alloc_quick(void){
	...
	mutex_lock(&quick_pte_map_mutex);
	if(list_empty(&quick_pte_map_head)){
		mutex_unlock(&quick_pte_map_mutex);
		return NULL;
	}
...
int list_pte_map_add(pte_t* ptep,struct list_head* head){
	struct sclock_page_pte_map* map;
	map=sclock_pte_map_alloc_quick();
	if(map==NULL)
	  map=kmem_cache_alloc(sclock_page_pte_map_cache,GFP_KERNEL);
	map->ptep=ptep;
	list_add(&map->head,head);
	return 0;
}
/*self pte mapping*/
int do_page_count_0(struct sclock_LRU * sclock_entry, pteval_t flags){
	struct list_head* pte_head;
	int  count=0;
	struct sclock_page_pte_map* pte_map,*n;
	unsigned long pfn=sclock_entry->pfn;
	...
	if(pte_pfn(*(sclock_entry->ptep))==pfn)
	  if(pte_present(*(sclock_entry->ptep)))
		if(sclock_entry->ptep->pte&flags){
			// sclock_entry->ptep->pte&=!flags;
			count++;
		}
	return 	count;
...
unsigned long long global_time1,global_time2;
unsigned long long global_interval=0,global_interval_reverse=0,global_interval_scan=0,global_interval_normal=0,global_interval_early=0,global_interval_got=0,global_interval_add=0,global_interval_define=0;
unsigned long long global_count=0,global_count_reverse=0,global_count_scan=0,global_count_add=0,global_count_got=0,global_count_normal=0,global_count_early=0;
...
__releases(ptl)
{
	...
	//	unsigned long address_one;
	struct sclock_LRU* sclock_entry,*sclock_UC_entry;
	//struct list_head *pid_ns->sclock_lru;
	...
	unsigned long FN,pfn_one;	/* For mmu_notifiers */
	page_one=NULL;
	page=NULL;
	pte_entry=orig_pte;
	if(update_protection_cache(0)<=0){
...
//	page=pfn_to_page(FN);
	page= vm_normal_pfn_to_page(vma, address, orig_pte,FN);
	//(vma,address,orig_pte);
	//	old_page = vm_normal_page(vma, address, orig_pte);
	//	printk(KERN_DEBUG"source PFN=%lx\n",page_to_pfn(old_page));
	/*if (!page||!page_mapped(page)) {
		//printk("vm->flags=%lx,pte=%lx",vma->vm_flags,orig_pte);
		//	ptep_clear_flush(vma, address, page_table);
		//	printk("NULL page->unlock for pfn=%lx, pte=%lx\n,address=%lx",FN,orig_pte.pte,address);
		goto setearly;
	}*/
	//gotten:
	//		printk("----------------prepare to change----------------\n");
	//	pte_unmap_unlock(page_table, ptl);
	//	//printk("unlock");
	set_number=FN&(NPageColor-1);
	global_interval_define+=get_rdtsc()-global_time1;
	pid_ns=ns_of_pid(task_pid(mm->owner));
	...
	//	seq++;
		if(sclock_entry->pfn==FN){
			...
			spin_unlock(&(pid_ns->sclock_lock[set_number]));
			page_table=	 pte_offset_map_lock(mm, pmd, address, &ptl);
			if(!(page_table->pte&_PAGE_CACHE_PROTECT)||pte_pfn(*page_table)!=FN)
			  goto unlock;
			page_table->pte&=~_PAGE_CACHE_PROTECT;
			flush_tlb_page(vma,address);
			spin_unlock(ptl);
			list_pte_map_add(page_table,&sclock_entry->pte_map);
		//	global_interval_scan+=get_rdtsc()-global_time2;
			goto out;
			//continue;
		}
		...
		pfn_one=sclock_entry->pfn;
		//	one_lru=&(sclock_entry->sclock_lru);
		page_one=pfn_to_page(pfn_one);
		if((!page_one)||(!page_mapped(page_one))){
			del_candidate=&sclock_entry->sclock_lru;
			findNToDel=1;
			set_protect=false;
			//goto find_del;
	...
	//	if(page_mapcount(page_one)>0)
	//	  atomic_add(do_page_count(page_one,pid_ns,_PAGE_CACHE_PROTECT),&sclock_entry->access_times);
	}
...
find_del:
	...
	do_page_set(page_one,pid_ns,_PAGE_CACHE_PROTECT, 10000);
	clflush_all(kmap(pfn_to_page(sclock_entry->pfn)),PAGE_SIZE);
	kunmap(pfn_to_page(sclock_entry->pfn));
add:
	//	global_time2=get_rdtsc();
	page_table=	 pte_offset_map_lock(mm, pmd, address, &ptl);
	if(!(page_table->pte&_PAGE_CACHE_PROTECT)||pte_pfn(*page_table)!=FN){
...
early_add:
	...
	if(addNewLRU){
		...
		spin_lock(&(pid_ns->sclock_lock[set_number]));
		mkOneScolor(sclock_entry,FN);
		sclock_entry->ptep=page_table;
		list_add_tail(&sclock_entry->sclock_lru,lru_head);
		spin_unlock(&(pid_ns->sclock_lock[set_number]));
		//	list_pte_map_add(page_table,&sclock_entry->pte_map);
	}
	global_interval+=get_rdtsc()-global_time1;
	global_count_normal++;
	return ret;
setearly:
	page_table->pte&=~_PAGE_CACHE_PROTECT;	
	//	set_pte_at(mm, address, page_table, pte_entry);
	if(invalid)
	  flush_tlb_page(vma,address);
	// mmu_notifier_invalidate_page(mm, address);
unlock:
...
out:
	global_count_early++;
	global_interval_early+=get_rdtsc()-global_time1;
	return ret;
}



static int _try_switch_NCache_copy(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid)
{
	global_count++;
	global_time1=get_rdtsc();
	//	struct task_struct* task=current;
...
//	unsigned long address_one;
	...
	page=NULL;
	pte_entry=orig_pte;
	if(update_protection_cache(0)<=0||mm->def_flags&VM_CACHE_PROTECT==0){
		goto setearly;
	}
	FN=pte_pfn(orig_pte);
	/*if(!pfn_valid(FN)){
...
//	page=pfn_to_page(FN);
	...
	//	ptep_clear_flush(vma, address, page_table);
	//pte_entry.pte&=~_PAGE_CACHE_PROTECT;
	page_table->pte&=~_PAGE_NCACHE;
	pte_unmap_unlock(page_table, ptl);
	global_interval_got+=get_rdtsc()-global_time1;
	global_count_got++;
	//	unlock_page(page);
	...
	//printk("mapcount==%d\n",page_mapcount(page));
	if(PageAnon(page)){
	do_anon_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT);
	//printk("map=%d",	do_anon_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT));
	}
	else if(PageKsm(page)){
	do_ksm_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT);
	//printk("map=%d",	do_ksm_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT));
	}	
	else if(page_mapping(page)){
					do_file_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT);
				//printk("map=%d",	do_file_page_clear(page,pid_ns,_PAGE_CACHE_PROTECT));
			}
		}
	}*/
	//printk(KERN_DEBUG"address=%lx,FN=%lx,set NO. =%d\n,pte=%lx,orig_pte=%lx,NPageColor=%d,PAGE_MASK=%lx",address,FN,set_number,page_table->pte,orig_pte.pte,NPageColor,PAGE_MASK);
	if(pid_ns->sclock_lru_counter==NULL||pid_ns->sclock_lru==NULL){
		goto out;
	}
	counter=&(pid_ns->sclock_lru_counter[set_number]);
	...
	//	seq++;
		...
		//	one_lru=&(sclock_entry->sclock_lru);
		page_one=pfn_to_page(pfn_one);
		if((!page_one)||(!page_mapped(page_one))||((page_one->flags&PAGE_FLAGS_CHECK_AT_PREP)==0)){
			//printk("del !page_one\n");
			del_candidate=&sclock_entry->sclock_lru;
			findNToDel=1;
			set_protect=false;
...
find_del:
	...
	spin_unlock(&(pid_ns->sclock_lock[set_number]));
	global_interval_scan+=get_rdtsc()-global_time2;
	if(!set_protect||!page_one||(!page_mapped(page_one))||((page_one->flags&PAGE_FLAGS_CHECK_AT_PREP)==0)){
		goto add;
	}
	//		for(i=0;i<findNToDel;i++){
	//	sclock_entry=list_entry(del_candidate,struct sclock_LRU,sclock_lru);
	...
	global_time2=get_rdtsc();
	do_page_set(page_one,pid_ns,_PAGE_CACHE_PROTECT,pte_count);
	/*	if(PageKsm(page_one)){
	//printk("set for mapped =%d",do_ksm_page_set(page_one,pid_ns,_PAGE_CACHE_PROTECT));
	do_ksm_page_set(page_one,pid_ns,_PAGE_CACHE_PROTECT);
	}	
	...
	global_interval_reverse+=get_rdtsc()-global_time2;
	global_count_reverse++;
	//	unlock_pag(page_one);
	//	}
	//	}
	//	}
...
add:
	...
	global_interval_add+=get_rdtsc()-global_time2;
	global_count_add++;
	//	atomic_sub(findNToDel-addNewLRU,counter);
	/*	if(page){
		page_cache_release(page);
	...
	/*	if (mmun_end > mmun_start)
		mmu_notifier_invalidate_range_end(mm, mmun_start, mmun_end);
		*/	
	global_interval_normal+=get_rdtsc()-global_time1;
	global_interval+=get_rdtsc()-global_time1;
	page_table->pte&=~_PAGE_CACHE_UC_MINUS;
	if(invalid)
	  mmu_notifier_invalidate_page(mm, address);
	global_count_normal++;
	return ret;
...
static int _try_switch_NCache_pte(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid)
{
	global_count++;
	global_time1=get_rdtsc();
	//	struct task_struct* task=current;
...
//	pte_count=atomic_read(&sclock_entry->pte_count);
	...
	spin_unlock(&(pid_ns->sclock_lock[set_number]));
	global_interval_scan+=get_rdtsc()-global_time2;
	if(!set_protect||!page_one||(!page_mapped(page_one))||((page_one->flags&PAGE_FLAGS_CHECK_AT_PREP)==0)||pte_pfn(*ptep_one)!=pfn_one){
		goto add;
	}
	//		for(i=0;i<findNToDel;i++){
	//	sclock_entry=list_entry(del_candidate,struct sclock_LRU,sclock_lru);
	...
	global_time2=get_rdtsc();
	ptep_one->pte|=_PAGE_CACHE_PROTECT;
	//mmu_notifier_invalidate_page(sclock_entry->mm, sclock_entry->address);
...
static int _try_switch_NCache_original(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid)
__releases(ptl)
{
	global_count++;
	global_time1=get_rdtsc();
...
int manage_cacheability(struct mm_struct *mm, struct vm_area_struct *vma,
	...
	if(trigger_by_NCache(pte)==1){
		//	//printk("triggerd by NCache\n");
		if(!(mm->def_flags&VM_CACHE_PROTECT)||!(vma->vm_flags&VM_CACHE_PROTECT)){
			if(mm->def_flags&VM_CACHE_PROTECT){
				vma->vm_flags|=VM_CACHE_PROTECT;
			}else{
				//printk("!!!!!!!!!not a NCache process:%s\n",mm->owner->comm);
				//entry=*page_table;
				entry.pte&=~_PAGE_CACHE_PROTECT;
				flush_tlb_page(vma, address);
...
static inline int handle_double_cache_pte_fault(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid) 
{
	int ret=VM_FAULT_RSVD;
	/*	if(mm->def_flags&VM_ISOLATION){
