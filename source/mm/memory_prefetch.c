...
__releases(ptl)
{
	...
	unsigned long FN,pfn_one,flags;	/* For mmu_notifiers */
//	struct sclock_page_pte_map* pte_map;//reverse
	...
	flush_tlb_page(vma,address);
	pte_unmap_unlock(page_table, ptl);
	//pte_map=list_pte_map_create(page_table,vma,address);//reverse
	spin_lock(&(pid_ns->sclock_lock[set_number]));
	...
	if(test_bit(PG_cacheable,&page->flags)){
		if(sclock_control->debug==1){
			printk("[in queue]page %lx already in queue in set %d for ns=%lx,address=%lx,task=%s,pte=%lx\n",FN,set_number,pid_ns,address,mm->owner->comm,page_table);
		}
		findNToDel=0;
		//		list_move_tail(&sclock_entry->sclock_lru,lru_head);
		page_table=	 pte_offset_map_lock(mm, pmd, address, &ptl);
	...
	if(list_empty(lru_head)){
		printk("abnormal list empty for address=%lx,fn=%lx, set_number=%d, address=%lx,task=%s\n",address,FN,set_number,address,mm->owner->comm);
		spin_unlock(&(pid_ns->sclock_lock[set_number]));			
		int newk=get_k_of_ns(pid_ns)+1;
		set_k_of_ns(pid_ns,newk);
		page_table=pte_offset_map_lock(mm, pmd, address, &ptl);
		if(!(page_table->pte&_PAGE_CACHE_PROTECT)||pte_pfn(*page_table)!=FN)
		{
			printk("solved\n");
...
find_del:
	...
	spin_unlock(&(pid_ns->sclock_lock[set_number]));
	if(sclock_control->debug==1){
		printk("[replace]page %lx replace page %lx in queue in set %d for ns=%lx, address=%lx,task=%s,pte=%lx\n",FN,sclock_entry->pfn,set_number,pid_ns,address,mm->owner->comm,page_table);
	}
	if(page_one)
	  clear_bit(PG_cacheable,&page_one->flags);
	if(page_one&&page_mapped(page_one)&& page_rmapping(page_one)){
		int welock=true;
		unsigned long time2=get_rdtsc();
		if(!trylock_page(page_one))
		{
		...
		if(welock)
		  unlock_page(page_one);
		//	}
		//		clean_sclock_pte_map(sclock_entry);//reverse
	//	__get_cpu_var(global_interval)+=get_rdtsc()-time2;
...
static int _try_switch_NCache_ins(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid)
__releases(ptl)
{
	unsigned long long time1;
	time1=get_rdtsc();
	//	struct task_struct* task=current;
	struct page* page,*page_one,*page_ones[8];
	atomic_t * counter,*UC_counter;
	struct sclock_LRU* sclock_entry;
 struct sclock_LRU* sclock_entrys[8];
	struct list_head * ins_lru_head,*lru_head,* del_candidate;
	pte_t pte_entry;
	struct pid_namespace* pid_ns=NULL;
	int i,ret=0,set_number,findNToDel=0,pte_count,seq=0;
	bool set_protect=true,addNewLRU=true;
	unsigned long FN,pfn_one,flags;	/* For mmu_notifiers */
	struct sclock_page_pte_map* pte_map;
	bool isIns=!(page_table->pte&_PAGE_NX);
	page_one=NULL;
	page=NULL;
	pte_entry=orig_pte;
	if(sclock_control->action&request_lru==0){
	...
	counter=&(pid_ns->sclock_lru_counter[set_number]);
	lru_head=&(pid_ns->sclock_lru[set_number]);
	ins_lru_head=&(pid_ns->sclock_ins_lru[set_number]);
	if(atomic_read(counter)+atomic_read(&(pid_ns->sclock_ins_lru_counter[set_number]))<get_k_of_ns(pid_ns)){
		spin_unlock(&(pid_ns->sclock_lock[set_number]));
		page_table=pte_offset_map_lock(mm, pmd, address, &ptl);
		if(!(page_table->pte&_PAGE_CACHE_PROTECT)||pte_pfn(*page_table)!=FN)
		{
		...
		atomic_set(counter,atomic_read(counter)|OVER_COUNTER_MASK);
	}*/
	if(isIns){
		list_for_each_entry(sclock_entry,ins_lru_head,sclock_lru){
			if(sclock_entry->pfn==FN){
				addNewLRU=0;
					if(sclock_control->debug){
					printk("[in queue]page %lx already in queue in set %d for ns=%lx\n",FN,set_number,pid_ns);
					}

				findNToDel=0;
				//	pte_map=list_pte_map_create(page_table,vma,address); 
				page_table=	 pte_offset_map_lock(mm, pmd, address, &ptl);
				pte_map->ptep=page_table;
				list_pte_map_add(pte_map,sclock_entry);
				spin_unlock(&(pid_ns->sclock_lock[set_number]));
				if(!(page_table->pte&_PAGE_CACHE_PROTECT)||pte_pfn(*page_table)!=FN)
				  goto unlock;
				page_table->pte&=~_PAGE_CACHE_PROTECT;
	...
	if(list_empty(lru_head)){
		printk("abnormal list empty for address=%lx,fn=%lx, set_number=%d\n",address,FN,set_number);
		spin_unlock(&(pid_ns->sclock_lock[set_number]));			
		int newk=get_k_of_ns(pid_ns)+1;
		set_k_of_ns(pid_ns,newk);
		page_table=pte_offset_map_lock(mm, pmd, address, &ptl);
	...
	if(sclock_control->debug)
	  findNToDel=sclock_control->debug;
	else
	  findNToDel=1;
find_del:

	for(i=0;i<findNToDel;i++){
		if(!isIns||i>1||list_empty(ins_lru_head)||get_random_number()%10<8){
			sclock_entry=list_entry(lru_head->next,struct sclock_LRU,sclock_lru);
			sclock_entrys[i]=sclock_entry;			
			if(pfn_valid(sclock_entry->pfn)){
				page_ones[i]=pfn_to_page(sclock_entry->pfn);
			}else{
				page_ones[i]=NULL;
			}
			list_del_init(&sclock_entry->sclock_lru);
			...
			sclock_entry=list_entry(ins_lru_head->next,struct sclock_LRU,sclock_lru);
			sclock_entrys[i]=sclock_entry;			
			if(pfn_valid(sclock_entry->pfn)){
				page_ones[i]=pfn_to_page(sclock_entry->pfn);
			}else{
			...
			list_del_init(&sclock_entry->sclock_lru);
			atomic_dec(&(pid_ns->sclock_ins_lru_counter[set_number]));
		}
	}
	spin_unlock(&(pid_ns->sclock_lock[set_number]));
			if(sclock_control->debug){
		printk("[replace]page %lx replace page %lx in queue in set %d for ns=%lx\n",FN,sclock_entry->pfn,set_number,pid_ns);
	}

		for(i=0;i<findNToDel;i++){
			page_one=page_ones[i];
			sclock_entry=sclock_entrys[i];			
			if(page_one&&page_mapped(page_one)&& page_rmapping(page_one)){
				int welock=true;
				unsigned long time2=get_rdtsc();
				/*if(!trylock_page(page_one))
				...
				/*	clean_sclock_pte_map(sclock_entry);
					__get_cpu_var(global_interval)+=get_rdtsc()-time2;
					__get_cpu_var(global_count)++;
					if(__get_cpu_var(global_count)>100000){
					__get_cpu_var(global_count_total)=__get_cpu_var(global_count_total)+1;
					__get_cpu_var(global_interval)=__get_cpu_var(global_interval)/ __get_cpu_var(global_count);
					__get_cpu_var(global_count)=1;
					}*/
				//	spin_lock(&(pid_ns->sclock_lock[set_number]));
			}
		}
		...
		list_pte_map_add(pte_map,sclock_entry);
		spin_lock(&(pid_ns->sclock_lock[set_number]));
		if(isIns&&atomic_read(&(pid_ns->sclock_ins_lru_counter[set_number]))<get_k_of_ns(pid_ns)/2){
			list_add_tail(&sclock_entry->sclock_lru,&(pid_ns->sclock_ins_lru[set_number]));
			atomic_inc(&(pid_ns->sclock_ins_lru_counter[set_number]));
		}else{
			list_add_tail(&sclock_entry->sclock_lru,&(pid_ns->sclock_lru[set_number]));
			atomic_inc(&(pid_ns->sclock_lru_counter[set_number]));
		}
		spin_unlock(&(pid_ns->sclock_lock[set_number]));
		if(sclock_control->debug){
		printk("[add]page %lx enqueued in set %d for ns=%lx\n",FN,set_number,pid_ns);
	}
		//	if(!(page_table->pte&_PAGE_NCACHE))
		//	global_count_early++;   
		//	global_interval_early+=get_rdtsc()-time1;
...
int prefetch_next(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid){
...
DEFINE_PER_CPU(struct task*,prefetch_thread)=NULL;
DEFINE_PER_CPU(struct paraPrefetch,prefetch_para);
DEFINE_PER_CPU(wait_queue_head_t,prefetch_thread_wait)=__WAIT_QUEUE_HEAD_INITIALIZER(prefetch_thread_wait);
DEFINE_PER_CPU(struct mutex,prefetch_thread_mutex)=__MUTEX_INITIALIZER(prefetch_thread_mutex);
struct paraPrefetch para;
int prefetch_thread_fn(void * data){
	set_freezable();
	int cpuid=*((int*)data);
	set_user_nice(current, 5);
	while (!kthread_should_stop()) {
		mutex_lock(&per_cpu(prefetch_thread_mutex,cpuid));
		struct paraPrefetch* parap=&(per_cpu(prefetch_para,cpuid));
		printk("para=%lx",parap);
		prefetch_pte(parap->mm,parap->vma,parap->address,parap->page_table,parap->pmd,parap->ptl,parap->orig_pte,parap->invalid);
		mutex_unlock(&(per_cpu(prefetch_thread_mutex,cpuid)));
		try_to_freeze();
		wait_event_freezable(per_cpu(prefetch_thread_wait,cpuid),kthread_should_stop());
	}
	return 0;
}
...
static int handle_double_cache_pte_fault(struct mm_struct *mm, struct vm_area_struct *vma,unsigned long address, pte_t *page_table, pmd_t *pmd, spinlock_t *ptl, pte_t orig_pte,bool invalid) 
{
	...
	if(sclock_control->debug>2){
		if(mm->owner)
		{
			if((strstr(mm->owner->comm,"run")==NULL)&&(strcmp(mm->owner->comm,"canneal")==0))
			  return 0;
		}else{
			return 0;
		}
		...
		struct paraPrefetch* para=&(per_cpu(prefetch_para,cpuid));
		printk("para=%lx",para);
		para->mm=mm;
		para->vma=vma;
		para->address=address;
		para->page_table=page_table;
		para->pmd=pmd;
		para->ptl=ptl;
		para->orig_pte=orig_pte;
		para->invalid=invalid;

		if(sclock_control->debug>5)
		  printk("create one\n");
		if((per_cpu(prefetch_thread,cpuid)==NULL)){
			per_cpu(prefetch_thread,cpuid)=kthread_run(prefetch_thread_fn,&cpuid,"prefetch_%d",cpuid);
		}else{
			wake_up_interruptible(&per_cpu(prefetch_thread_wait));
		}
	}
	if((find_prefetch_map(mm->owner->comm)!=NULL)&&(sclock_control->debug==3)){
