...
/*static int get_ksm_copy_page_state(struct page* page,pteval_t flags,	struct vm_area_struct* vma){
				...
				if(pte){	
					if(pte->pte&flags){
						...
						if(pte->pte&flags){
							...
							merge_into_one_page(vma,page,source);//remap pte to source;
						}
					}
				}
			}
...
del_one_copy_lock:
					 ...
					 case 0:
						 ...
						 //	printk("changed original,pfn=%lx,orig=%lx",coa_parent->pfn,orig_pfn);
...
static int sclockd__thread(void *nothing)
{
	...
	while (!kthread_should_stop()) {
		...
		try_to_freeze();
		if(sclockd_should_run(request_coa)){
			schedule_timeout_interruptible( msecs_to_jiffies(sclock_thread_sleep_millisecs(0)));
		}else{
...
static int __init sclockd_init(void){
	struct task_struct * sclock_thread,*lru_thread;
	int err=0;
