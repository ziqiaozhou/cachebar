...
bool is_new=false;
	...
	if (page == swapcache)
		page_add_anon_rmap_lock(page, vma, addr);
...
bad_file:
	pfn=pte_pfn(swp_entry_to_pte(entry));
	 page=pfn_to_page(pfn);
	pr_err("swap_dup: %s%08lx,%d,%d,pte=%lx,pfn=%lx,page->flag=%lx\n", Bad_file, entry.val,nr_swapfiles,type,swp_entry_to_pte(entry).pte,pfn,page->flags);
	goto out;
}
...
{
	if(__swap_duplicate(entry, SWAP_MAP_SHMEM)<0){
		printk("error shmem_alloc\n");
	}
}
...
int swapcache_prepare(swp_entry_t entry)
{
	...
	int ret=__swap_duplicate(entry, SWAP_HAS_CACHE);
	if(ret<0)
	  printk("error prepare\n");
	return ret;
}
